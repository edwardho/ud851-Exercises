...
public class ChildActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (startedActivityIntent.hasExtra(Intent.EXTRA_TEXT)) {

            // COMPLETE (5) If the Intent contains the correct extra, retrieve the text
            String intentText = startedActivityIntent.getStringExtra(Intent.EXTRA_TEXT);

            // COMPLETE (6) If the Intent contains the correct extra, use it to set the TextView text
            mDisplayText.setText(intentText);
        }
    }
}